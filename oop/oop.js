...
class Patient {
  constructor(typePatient, name, age, gender, budgetMaxPatient, diagnosis, totalDayTreatment, room, isCheckOut) {
    this.typePatient = typePatient
    this.name = name
    this.age = age
    this.gender = gender
    this.budgetPatient = budgetMaxPatient
...
class TypeOfpatient {
  constructor(fileData) {
    this.dataPatient = fileData
    this.patient = this.checkIn()
  }

  checkIn() {
    var patientData = fs.readFileSync('patientData.TXT', 'utf8')
    var splitPatient = patientData.split('\r\n')
    // console.log(splitPatient);
    var objectPatient = []

    for (var i = 1; i < splitPatient.length - 1; i++) {
      var obj = splitPatient[i].split(',')
      var objData = new Patient(obj[0], obj[1], obj[2], obj[3], obj[4], obj[5], obj[6], obj[7], obj[8])
      objectPatient.push(objData)
    }
    // for (var i = 0; i < objectPatient.length; i++) {
    //   if (true) {
    //
    //   }
    // }
...
let dataPatient = new TypeOfpatient('patientData.TXT')
console.log(dataPatient.checkIn());
// dataPatient.roomType()
